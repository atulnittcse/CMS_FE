...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private fun setUI() {
        ...
        taskAdapter.setOnItemClickListenerForCleanData {
            Constants.curCleaningData = it
            Navigation.findNavController(requireActivity(),R.id.nav_host_fragment_content_main).navigate(R.id.action_homeFragment_to_submitFragment)
        }
    ...
    fun showDatePickerDialog(){
        ...
        datePicker.addOnPositiveButtonClickListener {
            val format = getDate(it)
            val dayFormat = SimpleDateFormat("EEEE")
            val filterFormat = SimpleDateFormat("dd/MM/yyyy")
            binding.tvDate.text = format.format(it)
            binding.tvDay.text = dayFormat.format(it)

            val filterDate = filterFormat.format(it)
            viewmodel.getDailyRecordByID(format)
    ...
    private fun getTaskList(): List<TaskData> {
        val shift1Occurrence1Task1 = CleaningData(1, occurrence = 1)
        val shift1Occurrence2Task1 = CleaningData(1, occurrence = 2)
        val shift1Occurrence3Task1= CleaningData(1, occurrence = 3)
        val shift2Occurrence1Task1 = CleaningData(2, occurrence = 4)
        val shift2Occurrence2Task1 = CleaningData(2, occurrence = 5)
        val shift3Occurrence1Task1 = CleaningData(3, occurrence = 6)
        val data1 =  TaskData("1","All Circulating area (North, South)",6, listOf(shift1Occurrence1Task1,shift1Occurrence2Task1,shift1Occurrence3Task1,shift2Occurrence1Task1,shift2Occurrence2Task1,shift3Occurrence1Task1))
       /// val data1 = TaskData("1","All Circulating area (North, South)",6, getCleanDataList(6))

        val shift1Occurrence1Task2 = CleaningData(1, occurrence = 1)
        val shift1Occurrence2Task2 = CleaningData(1, occurrence = 2)
        val shift2Occurrence1Task2 = CleaningData(2, occurrence = 3)
        val shift2Occurrence2Task2 = CleaningData(2, occurrence = 4)
        val shift3Occurrence1Task2 = CleaningData(3, occurrence = 5)
        val data2 = TaskData("2","All Circulating area (2nd entry south, Kachha road 2nd entry)",5,
            listOf(shift1Occurrence1Data2,shift1Occurrence2Data2,shift2Occurrence1Data2,shift2Occurrence2Data2,shift3Occurrence1Data2))

        val shift1Occurrence1Data3 = CleaningData(1, occurrence = 1)
        val shift1Occurrence2Data3 = CleaningData(1, occurrence = 2)
        val shift2Occurrence1Data3 = CleaningData(2, occurrence = 3)
        val shift3Occurrence1Data3 = CleaningData(3, occurrence = 4)
        val data3 = TaskData("3","All Offices (Station building at North & South side,Railtel, North coolie room, south side coolie room)",
            4,   listOf(shift1Occurrence1Data3,shift1Occurrence2Data3,shift2Occurrence1Data3,shift3Occurrence1Data3))
    ...
    fun getCleanDataList(shift : Int) : List<CleaningData>{
        ...
        for (data in 1..shift){
            cleanList.add(CleaningData(1,0,"", FILL, occurrence = data))
        }
        return cleanList
